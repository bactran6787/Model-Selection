...
#List for gg plot
cat_list <- c("BloodType")
num_list <- c("Alcohol","Coffee","Exercise","ChemoTreatments", "ReagentA", "ReagentB",
              "ReagentC", "ReagentD", "ReagentE", "ReagentF", "ReagentG", "ReagentH", "ReagentI",
              "ReagentJ", "ReagentK", "ReagentL", "ReagentM", "ReagentN", 'Response')
num_initial <- c("Alcohol","Coffee","Exercise","ChemoTreatments","ReagentB", "ReagentC", "ReagentH", "ReagentM", 'Response' )

#Preprocessing initial
glmnet_initial <- c("impute_knn","corr", "dow","month","dummy")  # <-- These are arbitrary starting values. Set these to your best recommendation
pls_initial <- c("impute_knn","corr", "dow","month","dummy")    
rpart_initial <- c("impute_knn","corr", "dow","month","dummy") 
rf_initial <- c("impute_knn","corr", "dow","month","dummy") 
svmLinear_initial <- c("impute_knn","corr", "dow","month","dummy") 
svmRadial_initial <- c("impute_knn","corr", "dow","month","dummy") 
cubist_initial <- c("impute_knn","corr", "dow","month","dummy")
enet_initial <- c("impute_knn","corr", "dow","month","dummy")
qrf_initial <- c("impute_knn","corr", "dow","month","dummy")
gaussprPoly_initial <- c("impute_knn","corr", "dow","month","dummy")
rvmPoly_initial <- c("impute_knn","corr", "dow","month","dummy")
svmPoly_initial <- c("impute_knn","corr", "dow","month","dummy")
M5_initial <- c("impute_knn","corr", "dow","month","dummy")
lm_initial <- c("impute_knn","corr", "dow","month","dummy")
gaussprLinear_initial <- c("impute_knn","corr", "dow","month","dummy")
rlm_initial <- c("impute_knn","corr", "dow","month","dummy")
bagEarthGCV_initial <- c("impute_knn","corr", "dow","month","dummy")
nnet_initial <- c("impute_knn","corr", "dow","month","dummy")
mlp_initial <- c("impute_knn","corr", "dow","month","dummy")
...
ppchoices <- c("impute_knn", "impute_bag", "impute_median", "impute_mode", "YeoJohnson", 
               "naomit", "pca", "pls", "ica", "center", "scale", "month", "dow", 
               "dateDecimal", "nzv", "zv","other", "dummy","poly", "interact", 
               "indicate_na", "corr", "poly_3")
...
dynamicSteps <- function(recipe, preprocess) {
  ...
  for (s in preprocess) {
    if (s == "impute_knn") {
      ...
      recipe <- recipes::step_poly(recipe, all_numeric_predictors(), degree = 3)
    } else if (s == "interact") {
